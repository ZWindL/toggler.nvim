--- Inspired by https://github.com/shortcuts/neovim-plugin-boilerplate/blob/main/lua/your-plugin-name/util/log.lua
-- TODO: The source file has a bug, don't forget to fix the upstream project
local log = {}
...
---
---@param scope string: the scope from where this function is called.
---@param level number|unknown: the log level of vim.notify.
---@param verbose boolean: when false, only prints when config.debug is true.
---@param str string: the formatted string.
---@param ... any: the arguments of the formatted string.
---@private
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.toggler.config ~= nil and not _G.toggler.config.debug then
        return
    end
